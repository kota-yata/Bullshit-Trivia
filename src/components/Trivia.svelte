    ...
    {#each data as { category, type, difficulty, question, correct_answer, incorrect_answer }, i}
      <div class="trivia-card">
        <div class="trivia-question-container">
          <span class="trivia-question">{question.replace(/&quot;/gi, '"').replace(/&#039;/gi, "'")}
            <span class="trivia-type">- ({type})</span></span>
        </div>
        {#if difficulty === 'easy'}
        ...
        {/if}
        {#if isAnswer[i]}
          <div class="trivia-answer" transition:scale="{{ duration: 500 }}">
            {correct_answer.replace(/&quot;/gi, '"').replace(/&#039;/gi, "'")}
          </div>
        {/if}
      </div>
    {/each}
